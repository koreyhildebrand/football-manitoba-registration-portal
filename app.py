...
# ====================== DYNAMIC AGE GROUP CALCULATION ======================
def calculate_age_group(dob_str, season_year):
    try:
        dob = datetime.datetime.strptime(str(dob_str).strip(), "%Y-%m-%d").date()
        birth_year = dob.year
        age_in_season = season_year - birth_year

        if 9 <= age_in_season <= 10:
            return "U10 Cruncher"
        elif 11 <= age_in_season <= 12:
            return "U12 Atom"
        elif 13 <= age_in_season <= 14:
            return "U14 PeeWee"
        elif 15 <= age_in_season <= 16:
            return "U16 Bantam"
        else:
...
# Apply dynamic age group based on current year (or selected year in Registrar)
current_year = datetime.date.today().year
if "Date of Birth" in players_df.columns:
    players_df["AgeGroup"] = players_df["Date of Birth"].apply(lambda x: calculate_age_group(x, current_year))
...
if authentication_status is True:
    ...
    # Re-calculate age groups based on current year
    if "Date of Birth" in players_df.columns:
    ...
    st.caption("✅ St. Vital Mustangs Registration Portal | Dynamic Age Groups + Logout Fixed")
